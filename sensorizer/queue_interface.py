...
class QueueEventHub(QueueInterface):
    ...
    def __init__(self, address, user, key):
        ...
        self.client_batch = EventHubClient(self.address, debug=False, username=self.user, password=self.key)
        self.sender = self.client_batch.add_sender()
        self.client_batch.run()

    def async_send(self, events: List[TimeserieRecord]):
        client = EventHubClientAsync(self.address, debug=True, username=self.user, password=self.key)
        ...
        with open(f"{self.filepath}/sensor.avro", 'a+b') as out:
            writer(out,
                   ...
                   [{'tag': e.tag,
                     'plant': e.plant,
                     'value': e.value,
                     'timestamp': e.ts} for e in events],
                   codec="deflate")
